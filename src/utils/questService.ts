...
export async function getActiveQuests(userId: string, guildId: string) {
...
export async function getCompletedQuests(userId: string, guildId: string) {
  const db = getDatabase();
  const questPath = `quests.${guildId}.${userId}`;
  const questData = await db.get(questPath) || { active: [], completed: [] };
...
export async function canStartQuest(userId: string, guildId: string, questId: string): Promise<{ canStart: boolean; reason?: string }> {
...
export async function startQuest(userId: string, guildId: string, questId: string): Promise<{ success: boolean; error?: string }> {
...
export async function updateQuestProgress(userId: string, guildId: string, type: string, amount: number = 1) {
  const db = getDatabase();
  const questPath = `quests.${guildId}.${userId}`;
  const questData = await db.get(questPath) || { active: [], completed: [] };
  
  if (!questData.active || questData.active.length === 0) return;
...
export async function checkQuestCompletion(userId: string, guildId: string): Promise<any[]> {
  const db = getDatabase();
  const questPath = `quests.${guildId}.${userId}`;
  const questData = await db.get(questPath) || { active: [], completed: [] };
  ...
  if (!questData.active) return completedQuests;
  
  questData.active = questData.active.filter((activeQuest: any) => {
    const quest = getQuest(activeQuest.questId);
    ...
    for (const [key, required] of Object.entries(quest.requirements)) {
      ...
      if (progress < (required as number)) {
        isComplete = false;
        break;
      }
    }
...
export async function giveQuestRewards(userId: string, guildId: string, quest: any) {
  const db = getDatabase();
...
export function formatQuest(quest: any, progress?: any): string {
  let text = `**${quest.name}**\n`;
  ...
  text += `\n🎁 Rewards:\n`;
  if (quest.rewards.coins) text += `  • 💰 ${quest.rewards.coins} coins\n`;
  if (quest.rewards.xp) text += `  • ⭐ ${quest.rewards.xp} XP\n`;
