...
export async function canCraft(userId: string, guildId: string, recipeId: string): Promise<{ canCraft: boolean; missing: any }> {
  ...
  for (const [materialId, required] of Object.entries(recipe.materials)) {
    ...
    if (have < (required as number)) {
      missing[materialId] = (required as number) - have;
      canCraft = false;
    }
  }
...
export async function craftItem(userId: string, guildId: string, recipeId: string): Promise<{ success: boolean; result?: any; error?: string; missing?: any }> {
  const recipe = getRecipe(recipeId);
  if (!recipe) {
  ...
  for (const [materialId, required] of Object.entries(recipe.materials)) {
    let removed = 0;
    inventory.items = inventory.items.filter((item: any) => {
      const itemId = item.id || item.itemId;
      if (itemId === materialId && removed < (required as number)) {
        removed++;
        return false;
      }
      return true;
...
export function getRecipesByLevel(level: number) {
  const recipes = getAllRecipes();
  return recipes.filter((recipe: any) => recipe.requiredLevel <= level);
}

export function formatRecipe(recipe: any): string {
