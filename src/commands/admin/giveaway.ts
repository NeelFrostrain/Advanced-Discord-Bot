...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction, client: ExtendedClient) {
    ...
    setTimeout(async () => {
      try {
        const fetchedMessage = await (interaction.channel as any)?.messages.fetch(message!.id);
        const reaction = fetchedMessage?.reactions.cache.get('🎉');

        if (!reaction) {
          return (interaction.channel as any)?.send('❌ No one entered the giveaway!');
        }
        ...
        if (entries.size === 0) {
          return (interaction.channel as any)?.send('❌ No valid entries for the giveaway!');
        }
        ...
        await (interaction.channel as any)?.send({ embeds: [winnerEmbed] });
      } catch (error) {
