  ...
  async execute(interaction: ChatInputCommandInteraction, client: ExtendedClient) {
    ...
    if (!background && !progressbar && !text && !accent) {
      ...
      const embed = EmbedFactory.leveling('Your Rank Card Settings')
        .addFields(
          { name: '🎨 Background', value: card.backgroundColor || '#2b2d31', inline: true },
          { name: '📊 Progress Bar', value: card.progressBarColor || '#5865f2', inline: true },
          { name: '📝 Text', value: card.textColor || '#ffffff', inline: true },
          { name: '✨ Accent', value: card.accentColor || '#5865f2', inline: true }
        )
    ...
    try {
      ...
      const embed = EmbedFactory.success(
        'Rank Card Updated!',
        'Your rank card has been customized successfully.'
      );
