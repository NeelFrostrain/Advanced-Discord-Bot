  ...
  async execute(interaction: ChatInputCommandInteraction, client: ExtendedClient) {
    ...
    try {
      ...
      const embed = EmbedFactory.custom(won ? '#00FF00' : '#FF0000', '🎰 Roulette')
        ...
        .addFields(
          { name: 'Your Bet', value: `${betType} - ${amount.toLocaleString()} coins`, inline: true },
          { name: 'Result', value: won ? `Won ${winnings.toLocaleString()} coins!` : `Lost ${amount.toLocaleString()} coins!`, inline: true },
