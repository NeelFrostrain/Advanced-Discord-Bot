...
function simulateBattle(playerHP: number, playerDamage: number, monsterHP: number, monsterDamage: number) {
  const log: string[] = [];
  let pHP = playerHP;
  let mHP = monsterHP;
  let turn = 1;
    ...
    try {
      ...
      if (lastHunt && now - lastHunt < cooldown) {
        ...
        const embed = EmbedFactory.warning(
          'Still Recovering',
          `⏰ Wait **${timeLeft}s** before hunting again.`
        );
      ...
      if (battle.won) {
        ...
        embed.addFields(
          { name: '💰 Coins Earned', value: `${coins.toLocaleString()}`, inline: true },
