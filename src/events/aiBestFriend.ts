...
export default {
  ...
  async execute(message: Message, client: ExtendedClient) {
    ...
    if (isReplyToBot && message.reference) {
      try {
        ...
        if (repliedMessage.author.id === client.user?.id) {
          ...
          userMessage = `[Previous conversation: You said "${repliedMessage.content.substring(0, 100)}..."]\n\nUser's reply: ${userMessage || message.content}`;
        }
      } catch (error) {
    ...
    if (!userMessage || userMessage.trim() === '') {
      await message.reply('Hey! You tagged me but didn\'t say anything! 😅 What\'s up?');
      return;
    }
    ...
    try {
      ...
      if (error.message?.includes('400')) {
        await message.reply('Hmm, the API didn\'t like that request 🤔 The admin might need to check the API key or setup!');
      } else if (error.message?.includes('429')) {
        await message.reply('Whoa, too many requests! 😅 Give me a minute to catch my breath!');
      } else {
        await message.reply('Oof, my brain just glitched for a sec! 😅 Can you say that again?');
      }
    }
  }
};
