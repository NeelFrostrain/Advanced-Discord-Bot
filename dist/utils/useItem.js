...
export async function useItem(userId, guildId, itemId, context = 'general') {
...
export function applyItemEffect(effect, battleState) {
    if (!effect || !battleState)
        return battleState;
    const newState = { ...battleState };
    switch (effect.type) {
        case 'heal':
            newState.playerHp = Math.min(newState.playerMaxHp || 100, (newState.playerHp || 100) + effect.value);
            break;
        case 'buff_attack':
...
export function updateBuffDurations(battleState) {
    if (!battleState.playerBuffs)
        return battleState;
    const newState = { ...battleState };
...
export function calculateBuffedStats(baseStats, buffs) {
...
export async function getUsableItems(userId, guildId, context = 'general') {
    const db = getDatabase();
    const inventoryPath = `inventory.${guildId}.${userId}`;
    const inventory = await db.get(inventoryPath) || { items: [] };
    const usableItems = [];
    ...
    for (const invItem of inventory.items) {
        ...
        if (context === 'battle' && !item.usableInBattle)
            continue;
        if (context === 'general' && !item.consumable && !item.usableInBattle)
            continue;
...
export function formatItemEffect(effect) {
    if (!effect)
        return 'No effect';
    switch (effect.type) {
        case 'heal':
