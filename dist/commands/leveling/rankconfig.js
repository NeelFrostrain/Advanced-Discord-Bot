    ...
    async execute(interaction, client) {
        ...
        try {
            ...
            if (subcommand === 'xp') {
                ...
                const embed = EmbedFactory.success('XP Updated', `Users will now gain ${amount} XP per message.`);
                ...
                await updateGuildRankConfig(interaction.guildId, config);
                const embed = EmbedFactory.success('Cooldown Updated', `XP cooldown set to ${seconds} seconds.`);
                await interaction.reply({ embeds: [embed] });
            }
                ...
                if (channel) {
                    ...
                    await updateGuildRankConfig(interaction.guildId, config);
                    const embed = EmbedFactory.success('Level Up Channel Set', `Level up messages will be sent to ${channel}.`);
                    await interaction.reply({ embeds: [embed] });
                }
                else {
                    config.levelUpChannel = undefined;
                    await updateGuildRankConfig(interaction.guildId, config);
                    const embed = EmbedFactory.success('Level Up Channel Removed', 'Level up messages will be sent in the same channel.');
                    await interaction.reply({ embeds: [embed] });
                }
            }
                ...
                if (multiplier === 0) {
                    ...
                    await updateGuildRankConfig(interaction.guildId, config);
                    const embed = EmbedFactory.success('Multiplier Removed', `Removed XP multiplier for ${role.name}.`);
                    await interaction.reply({ embeds: [embed] });
                }
                else {
                    config.multipliers[role.id] = multiplier;
                    await updateGuildRankConfig(interaction.guildId, config);
                    const embed = EmbedFactory.success('Multiplier Set', `${role.name} now has ${multiplier}x XP multiplier!`);
                    await interaction.reply({ embeds: [embed] });
                }
            }
            else if (subcommand === 'view') {
                const embed = EmbedFactory.leveling('Rank Configuration')
                    .addFields({ name: '⭐ XP Per Message', value: `${config.xpPerMessage}`, inline: true }, { name: '⏱️ Cooldown', value: `${config.xpCooldown / 1000}s`, inline: true }, { name: '📢 Level Up Channel', value: config.levelUpChannel ? `<#${config.levelUpChannel}>` : 'Same channel', inline: true });
