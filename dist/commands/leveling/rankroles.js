    ...
    async execute(interaction, client) {
        ...
        try {
            ...
            if (subcommand === 'add') {
                ...
                const existing = config.rankRoles.find((r) => r.level === level);
                if (existing) {
                ...
                config.rankRoles.sort((a, b) => a.level - b.level);
                await updateGuildRankConfig(interaction.guildId, config);
                const embed = EmbedFactory.success('Rank Role Added', `Users will now receive ${role.name} at level ${level}!`);
                await interaction.reply({ embeds: [embed] });
            }
            else if (subcommand === 'remove') {
                const level = interaction.options.getInteger('level', true);
                const index = config.rankRoles.findIndex((r) => r.level === level);
                if (index === -1) {
                    return interaction.reply({
                ...
                await updateGuildRankConfig(interaction.guildId, config);
                const embed = EmbedFactory.success('Rank Role Removed', `Removed ${removed.roleName} from level ${level}.`);
                await interaction.reply({ embeds: [embed] });
            }
