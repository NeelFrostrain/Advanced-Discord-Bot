    ...
    async execute(interaction, client) {
        ...
        try {
            ...
            const embed = EmbedFactory.custom('#FFD700', '🃏 Blackjack')
                .addFields({ name: 'Your Hand', value: `${formatHand(playerHand)} (${playerValue})`, inline: true }, { name: 'Dealer Hand', value: `${dealerHand[0].value}${dealerHand[0].suit} ??`, inline: true }, { name: 'Bet', value: `${bet.toLocaleString()} coins`, inline: true });
            if (playerValue === 21) {
                games.delete(gameId);
            ...
            const row = new ActionRowBuilder()
                .addComponents(new ButtonBuilder().setCustomId('bj_hit').setLabel('Hit').setStyle(ButtonStyle.Primary), new ButtonBuilder().setCustomId('bj_stand').setLabel('Stand').setStyle(ButtonStyle.Secondary));
                ...
                if (i.customId === 'bj_hit') {
                    ...
                    if (newValue > 21) {
                        ...
                        const hitEmbed = EmbedFactory.custom('#FFD700', '🃏 Blackjack')
                            .addFields({ name: 'Your Hand', value: `${formatHand(game.playerHand)} (${newValue})`, inline: true }, { name: 'Dealer Hand', value: `${game.dealerHand[0].value}${game.dealerHand[0].suit} ??`, inline: true }, { name: 'Bet', value: `${bet.toLocaleString()} coins`, inline: true });
