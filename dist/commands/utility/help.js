...
export default {
    ...
    async execute(interaction, client) {
        ...
        const embed = EmbedFactory.info('📚 Help Menu')
            .setDescription(`Welcome to the help menu! I have **${totalCommands}** commands across **${Object.keys(categories).length}** categories.\n\n` +
            '**Quick Navigation:**\n' +
            Object.entries(categories).map(([key, cat]) => `${cat.emoji} **${cat.name}** - ${cat.commands.length} commands`).join('\n') +
        ...
        const row = new ActionRowBuilder()
            ...
            .addOptions(Object.entries(categories).map(([key, value]) => ({
            label: value.name,
            value: key,
