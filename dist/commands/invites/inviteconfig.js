...
export default {
    data: new SlashCommandBuilder()
        ...
        .addChoices({ name: 'Fake Accounts', value: 'fakes' }, { name: 'Alt Accounts', value: 'alts' }, { name: 'Both', value: 'both' })
        .setRequired(true))
    ...
    async execute(interaction, client) {
        ...
        try {
            ...
            if (subcommand === 'autokick') {
                ...
                return interaction.reply({
                    embeds: [EmbedFactory.success('Auto-Kick Updated', `Auto-kick for ${type} is now ${enabled ? 'enabled' : 'disabled'}.`)]
                });
            }
            if (subcommand === 'view') {
                const embed = EmbedFactory.leveling('⚙️ Invite Tracking Configuration')
                    .addFields({ name: '📊 Tracking', value: config.trackingEnabled ? '✅ Enabled' : '❌ Disabled', inline: true }, { name: '📢 Log Channel', value: config.logChannelId ? `<#${config.logChannelId}>` : 'Not set', inline: true }, { name: '📅 Min Account Age', value: `${config.minAccountAge} days`, inline: true }, { name: '🚫 Auto-Kick Fakes', value: config.autoKickFakes ? '✅ Enabled' : '❌ Disabled', inline: true }, { name: '🚫 Auto-Kick Alts', value: config.autoKickAlts ? '✅ Enabled' : '❌ Disabled', inline: true }, { name: '⚠️ Suspicious Threshold', value: config.suspiciousJoinThreshold.toString(), inline: true });
